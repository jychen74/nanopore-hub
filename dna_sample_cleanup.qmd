...
::: {.callout-important}
本頁預設對象是 **DNA、PCR product、amplicon、post-PCR amplified cDNA**，也可用於 nucleic acid extraction kit 萃取後的 DNA cleanup。  
...
| nucleic acid extraction kit 後 cleanup | 適合 | 萃取後若懷疑 salts、guanidine、ethanol、column carryover 或其他抑制物，可用於去除雜質 |
...
| extraction kit 後去除雜質 | 1.0x 起始；低量或 fragmented DNA 才考慮 1.8x-2.0x | 萃取後 DNA；1.0x 對多數 gDNA / amplicon-sized DNA 已足夠 | salts、guanidine、ethanol、column carryover、enzyme inhibitors |
...
::: {.callout-note}
Extraction kit 後的 cleanup 若只是為了去除污染物，建議先從 **1.0x** 測試。  
`1.8x-2.0x` 比較適合 DNA 已 fragmented、樣本量很低且希望最大化 recovery 的情境；但較高 ratio 也會更容易保留短片段，不一定符合 Nanopore long-read 的目的。
:::
